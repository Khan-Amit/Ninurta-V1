...
class MainActivity : AppCompatActivity() {
        ...
        if (free < (arenaSize - headerSize) * 0.1) {
            RandomAccessFile(arenaFile, "rw").use { raf ->
                ...
                if (desire < 0.5f) {
                    ...
                    raf.seek(raf.filePointer + 8 + 4)
                    raf.read(lenBytes)
                    val sliceLen = bytesToInt(lenBytes) + 4 + 4 + 8 + 4
                    tail += sliceLen
                    saveHeaders()
                }
            }
    ...
    private fun readAllSlices() {
        ...
        tvStatus.text = result.ifEmpty { "No slices found" }
    }
    ...
    private fun floatToBytes(f: Float) = java.nio.ByteBuffer.allocate(4).putFloat(f).array()
    private fun bytesToFloat(b: ByteArray) = java.nio.ByteBuffer.wrap(b).float
    private fun longToBytes(l: Long) = java.nio.ByteBuffer.allocate(8).putLong(l).array()
    private fun bytesToLong(b: ByteArray) = java.nio.ByteBuffer.wrap(b).long
    private fun intToBytes(i: Int) = java.nio.ByteBuffer.allocate(4).putInt(i).array()
    private fun bytesToInt(b: ByteArray) = java.nio.ByteBuffer.wrap(b).int
}
